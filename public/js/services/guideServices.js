...
function($rootScope, User, $timeout, $state) {
  ...
  var steps = [
    {
      ...
      element: '.hero-stats',
      orphan: true,
      content: "You just gained Experience and Gold! Keep at it, you'll level up.",
      placement: 'bottom'
    },{
      element: ".hero-stats",
  ...
  _.each(steps, function(step) {
    ...
    if (!step.onShow) step.onShow = function(){
  ...
  intro.addSteps(steps.slice(0,2));
  $rootScope.$on('tourStep', function(data, step){
    switch(step) {
      case 'checkTodo':
        intro.addSteps(steps.slice(2,6));
        intro.restart();
        intro.goTo(2);
        break;
      case 'addDaily':
        intro.addSteps(steps.slice(6));
        intro.restart();
        intro.goTo(6);
        break;
    }
   ...
   */
  $rootScope.$on('userSynced', _.once(initTour));
  function initTour(){
    if (User.user.flags.showTour === false) return;
    $('.main-herobox').popover('destroy');
    intro.restart(); // Tour doesn't quite mesh with our handling of flags.showTour, just restart it on page load
    //tour.start(true);
  };
